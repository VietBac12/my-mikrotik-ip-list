...
def get_ips_smart(url, label, is_asn_source=False, is_vn_only=False):
    """Xử lý thông minh: Tự động nhận diện nguồn VN-Only"""
    ...
    try:
        ...
        for line in resp.text.splitlines():
            ...
            # 1. PHÂN LOẠI ISP (Từ nguồn ASN-Full)
            if is_asn_source:
            ...
            # 2. LỌC IP VIỆT NAM
            # Chấp nhận dòng nếu: (Nguồn đã là VN-Only) HOẶC (Dòng có chữ VN) HOẶC (APNIC VN)
            if is_vn_only or "VN" in line_raw or "apnic|VN|ipv4|" in line_raw:
                # Xử lý APNIC
...
def main():
    sources = [
        {"url": "https://ftp.apnic.net/stats/apnic/delegated-apnic-latest", "label": "APNIC", "asn": False, "vn": False},
        {"url": "https://raw.githubusercontent.com/herrbischoff/country-ip-blocks/master/ipv4/vn.cidr", "label": "GitHub VN", "asn": False, "vn": True},
        {"url": "https://raw.githubusercontent.com/sapics/ip-location-db/main/geolite2-country/geolite2-country-ipv4.csv", "label": "GeoLite2", "asn": False, "vn": False},
        {"url": "https://raw.githubusercontent.com/sapics/ip-location-db/main/iplocate-country/iplocate-country-ipv4.csv", "label": "iplocate-country", "asn": False, "vn": False},
        {"url": "https://raw.githubusercontent.com/sapics/ip-location-db/main/dbip-country/dbip-country-ipv4.csv", "label": "DB-IP", "asn": False, "vn": False},
        {"url": "https://raw.githubusercontent.com/firehol/blocklist-ipsets/master/ip2location_country/ip2location_country_vn.netset", "label": "IP2Location VN", "asn": False, "vn": True},
        {"url": get_latest_vnnic_url(), "label": "VNNIC", "asn": False, "vn": True},
        {"url": "https://raw.githubusercontent.com/sapics/ip-location-db/main/asn/asn-ipv4.csv", "label": "ASN-Full-Data", "asn": True, "vn": False}
    ]

    final = {"vn_ipv4": [], "vn_viettel": [], "vn_vnpt": [], "vn_fpt": [], "vn_mobifone": []}
    for src in sources:
        data = get_ips_smart(src['url'], src['label'], is_asn_source=src['asn'], is_vn_only=src['vn'])
    ...
    with open("vn_ipv4.rsc", "w") as f:
        f.write(f"# VN IP List - Corrected & Categorized - Updated: {datetime.datetime.now()}\n")
        for name, networks in final.items():
            merged = list(ipaddress.collapse_addresses(networks))
            print(f"[#] {name}: {len(merged)} dải IP.")
